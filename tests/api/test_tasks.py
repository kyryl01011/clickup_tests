...
from tests.conftest import authed_session, task_data
...
class TestTasks:
    ...
    def test_get_all_tasks(self, authed_session):
        ...
        assert type(all_tasks.json()['tasks']) is list, f'Unexpected type of response: {type(all_tasks.json()['tasks'])}, expected list'
    ...
    def test_get_task(self, authed_session, task_data):
        ...
        task_json = task.json()
        assert init_task_data['id'] == task_json['id'], f'Unexpected task id: {task_json['id']}, expected {init_task_data['id']}'
        assert init_task_data['name'] == task_json['name'], f'Unexpected task name: {task_json['name']}, expected {init_task_data['name']}'
    ...
    def test_successful_create_new_task(self, authed_session, task_data):
        ...
        assert verify_response_json['name'] == generated_task_data['name'], f'Unexpected task name: {verify_response_json['name']}, expected {generated_task_data['name']}'
        assert created_task_json['name'] == generated_task_data['name'], f'Unexpected task name: {created_task_json['name']}, expected {generated_task_data['name']}'
        return created_task_json

    @allure.description('Create task with wrong name, parent ID, link ID and validate that errors are equal to expected')
    @pytest.mark.parametrize(
        'invalid_data, expected_status_code, expected_err_message, expected_err_code',
        [
            ({'name': ''}, 400, 'Task name invalid', 'INPUT_005'),
            ({'name': DataGenerator.generate_random_word(), "parent": DataGenerator.generate_random_int()}, 400, 'Parent not child of list', 'ITEM_137'),
            ({'name': DataGenerator.generate_random_word(), "links_to": DataGenerator.generate_random_int()}, 401, 'You do not have access to this task', 'ACCESS_083')
        ]
    )
    def test_negative_create_new_task(self, authed_session, invalid_data, expected_status_code, expected_err_message, expected_err_code):
    ...
    @allure.description('Create new task with generated data, updates it and validate if not equal to initial and equal to newly generated')
    def test_successful_update_task(self, authed_session, task_data):
        ...
        assert new_task_data['name'] == updated_task_json['name'], f'Unexpected updated name: {updated_task_json['name']}, expected {new_task_data['name']}'
        assert initial_task_data['name'] != updated_task_json['name'], f'Initial data did not change: expected {new_task_data['name']}, got {updated_task_json['name']}'

    @allure.description('Generates random task ID and to access it with update request, validate to receive expected error message and code')
    def test_negative_update_task(self, authed_session, task_data):
        ...
        assert updated_task_json['err'] in ('Team not authorized', 'Team(s) not authorized'), f'Unexpected error message: {updated_task_json['err']}'
        assert updated_task_json['ECODE'] in ('OAUTH_023', 'OAUTH_027'), f'Unexpected error code: {updated_task_json['ECODE']}'

    @allure.description('Create new task with generated data and delete it by ID, validate status code and that response body is empty')
    ...
    @allure.description('Generate random task ID and try to delete it without permission, validate that received error message and code equal to expected')
    def test_negative_delete_task_by_id(self, authed_session, task_data):
        ...
        assert delete_task_response_json['err'] in ('Team not authorized', 'Team(s) not authorized'), f'Unexpected error message: {delete_task_response_json['err']}'
        assert delete_task_response_json['ECODE'] in ('OAUTH_023', 'OAUTH_027'), f'Unexpected error code: {delete_task_response_json['ECODE']}'